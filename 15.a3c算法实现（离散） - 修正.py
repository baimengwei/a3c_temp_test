...
class agent_master(mp.Process):
    ...
    def run(self):
        ...
        for i in range(self.epoch):
            ...
            old_critic = deepcopy(self.critic_network.model.state_dict())
            old_actor = deepcopy(self.actor_network.model.state_dict())
            ...
            if std_a < 0.06 or std_c < 0.5 or self.train_ok < 10:
                self.critic_network.optimizer.step()
                self.actor_network.optimizer.step()

            performance = self.perform_test(i)
            delta_E = performance - old_performance
            prob = np.exp(delta_E/Tempture)
            if prob < np.random.rand():
                self.critic_network.model.load_state_dict(old_critic)
                self.actor_network.model.load_state_dict(old_actor)
